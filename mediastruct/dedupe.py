...
class dedupe:
    ...
    def __init__(self, data_files, duplicates_dir, archive_dir, monitor=None):
        ...
        self.dups(combined_dataset, duplicates_dir, archive_dir)
        if self.monitor:
    ...
    def dups(self, array, duplicates_dir, archive_dir):
        ...
        # Prioritize keeping files in the archive
        ...
        for a, b, c in dictlist:
            ...
            if self.monitor and total_dictlist > 0:
                self.monitor.update_progress("dedupe", status="Running", processed=processed_dictlist, total=total_dictlist, current=f"Checking archive entry: {a}")
            log.debug(f"Dedupe - Checked {processed_dictlist}/{total_dictlist} entries for archive prioritization ({(processed_dictlist/total_dictlist)*100:.1f}%)")

        # Prune the list to exclude archive entries already marked to keep
        prunedict = [x for x in dictlist if x[0] not in to_keep]

        # Add non-archive files to keep list if they haven't been seen
        ...
        for a, b, c in prunedict:
            ...
            if self.monitor and total_prunedict > 0:
                self.monitor.update_progress("dedupe", status="Running", processed=processed_prunedict, total=total_prunedict, current=f"Checking non-archive entry: {a}")
            log.debug(f"Dedupe - Checked {processed_prunedict}/{total_prunedict} non-archive entries ({(processed_prunedict/total_prunedict)*100:.1f}%)")

        # Identify files to delete (duplicates not in archive)
        to_delete = [(x, y, z) for x, y, z in dictlist if x not in to_keep and archive_dir_name not in z]
            ...
            for ingest_file in ingest_dir.iterdir():
                ...
                if file_hash in archive_hashes:
                    ...
                    if os.path.isfile(dest_path):
                        ...
                        dest_path = os.path.join(duplicates_dir, newfilename)
        ...
        for k in range(total_to_delete):
            ...
            if os.path.isfile(from_path):
                ...
                if self.monitor and total_to_delete > 0:
                    self.monitor.update_progress("dedupe", status="Running", processed=processed_to_delete, total=total_to_delete, current=f"Moving duplicate: {filename}")
                log.debug(f"Dedupe - Moved {processed_to_delete}/{total_to_delete} duplicates ({(processed_to_delete/total_to_delete)*100:.1f}%)")
